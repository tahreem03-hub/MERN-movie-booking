...
const AddShows = () => {
  ...
  {/* to make the api call we need axios token and user */ }
  ...
  const handleSubmit = async () => {
    try {
      {/* it will disable add show button */ }
      setAddingShow(true)
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      {/* show price input */}
      <div className='mt-8'>
        <label className='block text-sm font-mediummb-2'>Show Price</label>
        ...
        px-3 py-2 rounded-md'>
          ...
          <input type="number" min={0} value={showPrice} onChange={(e) => setShowPrice(e.target.value)}
            placeholder='"EnterShow Price' className='outline-none' />
        </div>
      </div>
      ...
      <button
        ...
        disabled={addingShow}
        className='bg-primary text-white px-8 py-2 mt-6 roounded
